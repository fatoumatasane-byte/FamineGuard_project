...
# 1. FIX OBLIGATOIRE POUR CHROMADB SUR LE CLOUD
try:
...
def load_resources():
    ...
    try:
        ...
        # On force la recherche des noms de lieux (on évite les colonnes de dates)
        cols = df.columns.tolist()
        # On cherche une colonne qui contient des noms (Bakel, Matam...)
        # Souvent 'Area' ou 'area_name' ou 'title'
        for c in ['Area', 'area_name', 'zone', 'title', 'ADM2_FR']:
            if c in cols:
                df = df.rename(columns={c: 'nom_zone'})
                break
        if 'nom_zone' not in df.columns:
            df['nom_zone'] = df.iloc[:, 0] # Par défaut la 1ère colonne
    except:
    ...
    if os.path.exists('ipc_sen.geojson'):
        ...
        # On s'assure qu'il y a une colonne 'title' pour la correspondance
...
def agent_famine_guard(zone, phase, prix, ndvi, langue):
    try:
        ...
        return f"⚠️ Erreur de connexion au cerveau de l'agent. Recommandation pour {zone} : Activer l'aide alimentaire d'urgence immédiatement."

# --- 4. INTERFACE ---
with st.sidebar:
    st.header("🎮 Contrôles")
    # On affiche la liste des villes (triée)
    villes = sorted(nodes_df['nom_zone'].unique())
...
if run:
    ...
    with st.spinner("Analyse Agentique en cours..."):
        ...
        if senegal_map is not None:
            ...
            # On cherche la ville dans le GeoJSON
            mask = senegal_map['title'].str.lower().str.contains(zone_choisie.lower(), na=False)
            senegal_map.loc[mask, "color"] = "#E74C3C" # Rouge
